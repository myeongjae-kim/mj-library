...
bool isPrime(int N);
bool isAmicable(int N, unordered_map<int, int> &amicableNumbers);
...
{
    unordered_map<int, int> amicableNumbers;
    set<int> answers;
    ...
    for (int i = 1; i <= 10000; i++) {
        if(isAmicable(i, amicableNumbers)) {
            answers.insert(i);
            std::cout << i << std::endl;
        }
    }
...
bool isAmicable(int N, unordered_map<int, int> &amicableNumbers) {
    if(amicableNumbers[N] != 0)
        return true;
    
    int sumOfDiv1 = sumOfDivisors(N);
    int sumOfDiv2 = sumOfDivisors(sumOfDiv1);
    
    if(N == sumOfDiv2 && sumOfDiv1 != sumOfDiv2) { // if sumOfDiv1 == sumOfDiv2, than it is perfect number, not an amicable number
        amicableNumbers[N] = sumOfDiv1;
        amicableNumbers[sumOfDiv1] = N;
        return true;
    }
    return false;
}
